...
func awake() -> void:
	if(is_instance_valid($"Dropped Item")):
		$"Dropped Item/AnimatedSprite2D".visible = false
	if(get_tree().get_first_node_in_group("GameData").has_player_flag("room_completed_secret_to_everybody_001")):
		set_room_visibility(false)
		$fire.visible = true
		$fire2.visible = true
		has_item = true
		is_loaded = true
		$"Dropped Item".queue_free()
	else:
...
func _on_fire_animation_finished() -> void:
	...
	$fire.play("fire")
	$fire2.play("fire")
	if(!has_item and !get_tree().get_first_node_in_group("GameData").has_player_flag("room_completed_secret_to_everybody_001")):
...
func _on_key_item_item_picked_up() -> void:
	...
	get_tree().get_first_node_in_group("GameData").add_player_flag("room_completed_secret_to_everybody_001")
